...
Integrate all Cursor conversation capture components (watcher, parser, project detector, session tracker, exporter) into the main daemon process. This task wires together the complete pipeline so that file system events trigger the full conversation capture and export workflow.
...
5. Integrate markdown exporter to write files
6. Handle the complete pipeline: watcher → parser → project detector → session tracker → exporter
...
4. Implement capture service:
   - Initialize watcher, parser, project detector, session tracker, exporter
